...
impl StoreExt for StoreCtx<SqliteStore, StoreSqliteState, StoreSqliteOpts> {
    async fn init(&mut self) -> Result<()> {
        // Attempt to connect to the SQLite database. If the file doesn't exist, it will be created.
        // This writes to the state.
        self.connect().await?;
    ...
    async fn settings_fetch(&self) -> Result<Settings> {
        ...
        // We need to prepare a statement to fetch settings from the database. This will return a map of key-value pairs.
        let mut stmt = conn
